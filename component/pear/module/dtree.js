layui.define(['jquery','layer','form'], function(exports) {
    ...
    // 树的公共定义样式汇总
    const LI_NAV_CHILD = "dtree-nav-ul-sid", LI_NAV_ITEM = "dtree-nav-item",
    ...
    // 树的公共指定
    const NAV_THIS = "dtree-nav-this",	//当前节点
    ...
    // 树的自定义图标
    const DTREEFONT = "dtreefont",									//默认使用图标字体
    ...
    var AjaxHelper = {
        request : function(config) {
            ...
            const async = (typeof (config.async) === "boolean") ? config.async : true;
            $.ajax({
...
//			level: "level",			  //层级
    ...
    DTree.prototype.renderSelectDom = function() {
        const _this = this;
        const rootId = _this.obj[0].id;

        // 设置自定义表单隐藏域
        const selectInputName = _this.selectInputName;
        const selectInput = [];
        for(const key in selectInputName) {
        	selectInput.push('<input type="hidden" dtree-id="' + rootId + '" dtree-node="' + key + '" name="' + selectInputName[key] + '" value="" readonly>');
        }

        // 设置html
        const prevHtml = ['<div class="layui-unselect layui-form-select" dtree-id="' + rootId + '" dtree-select="' + _this.selectDiv + '">',
    ...
    DTree.prototype.reloadSelectDom = function() {
        ...
        const selectDiv = $("div[dtree-id='" + rootId + "'][dtree-select='" + _this.selectDiv + "']");
        selectDiv.find("div.layui-select-title").html("");

        // 设置html
        const prevHtml = [selectInput.join(""),
            '<input type="text" dtree-id="' + rootId + '" id="' + _this.selectTipsName + '_id" name="' + _this.selectTipsName + '" placeholder="' + _this.selectTips + '" value="" readonly class="layui-input layui-unselect">',
            '<i class="layui-edge"></i>'].join('');

        selectDiv.find("div.layui-select-title").html(prevHtml);
    ...
    DTree.prototype.selectVal = function(param) {
        ...
        const selectDiv = $("div[dtree-select='" + _this.selectDiv + "']");
    	for(const key in selectInputName) {
    		selectValues[selectInputName[key]] = param[key];
            selectDiv.find("input[dtree-id='" + rootId + "'][name='"+selectInputName[key]+"']").val(param[key] || "");
    	}
        ...
        // 返显提示输入框值
        selectDiv.find("input[dtree-id='" + rootId + "'][name='"+_this.selectTipsName+"']").val(selectTipsNameValue || "");
    ...
    DTree.prototype.selectCheckboxVal = function() {
        ...
        // 获取全部复选框选中节点
        const param = _this.getCheckbarJsonArrParam();

        selectTipsNameValue = param["context"];
        const selectValues = {};
        const selectDiv = $("div[dtree-select='" + _this.selectDiv + "']");
        for(const key in selectInputName) {
            const value = param[key].join(",");
            selectValues[selectInputName[key]] = value;
            selectDiv.find("input[dtree-id='" + rootId + "'][name='"+selectInputName[key]+"']").val(value);
    	}
    ...
    DTree.prototype.ensureTheme = function(){
        ...
        const iconfontStyle = this.iconfontStyle;
        let iconfontStyles = [];
        ...
        for(let i=0; i<iconfonts.length; i++){
            const ifont = iconfonts[i];
            const ifontStyle = iconfontStyles[i];
    ...
    DTree.prototype.useDefaultOrUserDefineFnodeStyle = function(ifont, fnode){
        const _this = this;
        const tempOpen = this.usefontStyle.fnode.node.open;
        const tempClose = this.usefontStyle.fnode.node.close;
        const tempLeaf = this.usefontStyle.fnode.leaf;
    ...
    DTree.prototype.useDefaultOrUserDefineSnodeStyle = function(ifont, snode){
        const _this = this;
        const tempOpen = this.usefontStyle.snode.node.open;
        const tempClose = this.usefontStyle.snode.node.close;
        const tempLeaf = this.usefontStyle.snode.leaf;
    ...
    DTree.prototype.useDefaultOrUserDefineCheckboxStyle = function(ifont, checkbox){
        const _this = this;
        const tempOn = this.usefontStyle.checkbox.on;
        const tempOut = this.usefontStyle.checkbox.out;
        const tempNoall = this.usefontStyle.checkbox.noall;
        ...
        if(typeof checkbox === 'undefined'){
            ...
            const on = checkbox.on;
            const out = checkbox.out;
            const noall = checkbox.noall;
    ...
    DTree.prototype.useDefaultOrUserDefineMenubarStyle = function(ifont, menubar){
        const _this = this;
        const tempMovedown = this.usefontStyle.menubar.movedown;
        const tempMoveup = this.usefontStyle.menubar.moveup;
        const tempRefresh = this.usefontStyle.menubar.refresh;
        const tempCheckAll = this.usefontStyle.menubar.checkAll;
        const tempUncheckAll = this.usefontStyle.menubar.unCheckAll;
        const tempInvertAll = this.usefontStyle.menubar.invertAll;
        const tempRemove = this.usefontStyle.menubar.remove;
        const tempSearch = this.usefontStyle.menubar.search;
        ...
        if(typeof menubar === 'undefined'){
            ...
            const movedown = menubar.movedown;
            const moveup = menubar.moveup;
            const refresh = menubar.refresh;
            const checkAll = menubar.checkAll;
            const unCheckAll = menubar.unCheckAll;
            const invertAll = menubar.invertAll;
            const remove = menubar.remove;
            const search = menubar.search;
    ...
    DTree.prototype.useDefaultOrUserDefineMenubarExtStyle = function(ifont, menubarExt){
        const _this = this;
        const tempExt = this.usefontStyle.menubarExt;
    ...
    DTree.prototype.useDefaultOrUserDefineToolbarStyle = function(ifont, toolbar){
        const _this = this;
        const tempMovedown = this.usefontStyle.toolbar.menubar.movedown;
        const tempMoveup = this.usefontStyle.toolbar.menubar.moveup;
        const tempRefresh = this.usefontStyle.toolbar.menubar.refresh;
        const tempCheckAll = this.usefontStyle.toolbar.menubar.checkAll;
        const tempUnCheckAll = this.usefontStyle.toolbar.menubar.unCheckAll;
        const tempInvertAll = this.usefontStyle.toolbar.menubar.invertAll;
        const tempRemove = this.usefontStyle.toolbar.menubar.remove;
        const tempSearch = this.usefontStyle.toolbar.menubar.search;
        const tempExt = this.usefontStyle.toolbar.menubarExt;
        const tempPulldown = this.usefontStyle.toolbar.pulldown;
        const tempPullup = this.usefontStyle.toolbar.pullup;
        const tempAdd = this.usefontStyle.toolbar.add;
        const tempEdit = this.usefontStyle.toolbar.edit;
        const tempDel = this.usefontStyle.toolbar.del;
        ...
        if(typeof toolbar === 'undefined'){
            ...
            const menubar = toolbar.menubar;
            const menubarExt = toolbar.menubarExt;
            const pulldown = toolbar.pulldown;
            const pullup = toolbar.pullup;
            const add = toolbar.add;
            const edit = toolbar.edit;
            const del = toolbar.del;
    ...
    DTree.prototype.useDefaultOrUserDefineToolbarExtStyle = function(ifont, toolbarExt){
        const _this = this;
        const tempExt = this.usefontStyle.toolbarExt;
    ...
    DTree.prototype.operateIcon = function($i_fnode, $i_snode){
        const _this = this;
        const ficonClass = $i_fnode.attr("data-iconClass");
        const iconClass = $i_snode.attr("data-iconClass");
        return{
            open: function(){
    ...
    DTree.prototype.showLineLi = function($li){
        ...
        if($ul[0].id === _this.obj[0].id) {
            ...
            // 非根节点下的节点
            const $pnextLi = $ul.parent("li").next("li");
            if($pnextLi.length === 0) {
                if($nextLi.length === 0){
                    $li.removeClass(LI_NAV_LINE);
                ...
                const $pnextdiv = $pnextLi.children("div");
    ...
    DTree.prototype.autoHeight = function(){
        ...
        if(height !== "") {
            if(_this.elem === _this.scroll){
                ...
                const $toolbarDiv = _this.obj.closest(_this.scroll);
    ...
    DTree.prototype.loadTreeInit = function(){
        const _this = this;
        const asyncLoad = _this.asyncLoad;
        // 初始化加载
    	_this.init(function(){
    		// ajax加载之后的回调
    ...
    DTree.prototype.loadChildTreeInit = function(asyncLoad, i){
        ...
        const $div = _this.getNode(asyncLoad[i]);
    ...
    DTree.prototype.getChild = function($div, data, callback) {
        const _this = this, $ul = $div.next("ul");
    ...
    DTree.prototype.dataLoadTree = function(first, $ul, data){
        ...
        const index = _this.load ? layer.load(1) : "";

        setTimeout(function () {
            // 加载完毕后执行树解析前的回调
            const returnData = _this.success(data, $ul, first);
            if(returnData) {
                data = returnData;
            }

            const pid = (first === true) ? $ul.attr("data-id") : _this.node.nodeId;
            const level = (first === true) ? 1 : parseInt(_this.node.level) + 1;

            // 解析树
            if (_this.dataFormat === 'list'){
                //1.构建一个存放节点的树组
                const rootListData = _this.queryListTreeByPid(pid, data);
    ...
    DTree.prototype.asyncLoadTree = function(first, $ul, callback){
        ...
        AjaxHelper.request({
            ...
            beforeSend: function(XMLHttpRequest, self){
                ...
                const returnFlag = _this.beforeSend(this, XMLHttpRequest, self);
                if(!returnFlag) {
            		if(_this.load){layer.close(index);}
            	}
            	return returnFlag;
            ...
            success: function(result) {
                ...
                // 加载完毕后执行树解析前的回调
                const returnData = _this.success(result, $ul, first);
                if(returnData) {
                    result = returnData;
                }
                ...
                if (code === _this.response.statusCode) {
                    ...
                    const d = result[_this.response.rootName];
                    ...
                    if (_this.dataFormat === 'list'){
                        //1.构建一个存放节点的树组
                        const rootListData = _this.queryListTreeByPid(pid, d);
                        if(first) {
    ...
    DTree.prototype.loadListTree = function(nowListData, listData, level, $ul){
        ...
        if (nowListData.length > 0){
            for (let i = 0; i < nowListData.length; i++) {
                // 1.获取已知节点的全部数据
                const data = nowListData[i];
                if(typeof data !== "object") continue;
                const parseData = _this.parseData(data);
                const childListData = _this.queryListTreeByPid(parseData.treeId(), listData); // 根据已知数据的id判断该条数据是否还有子数据
    ...
    DTree.prototype.msgErrData = function() {
        ...
        if(_this.errData.length > 0 && _this.errDataShow) {
            let title = "";
            for(let i=0; i<_this.errData.length; i++) {
                const edata = _this.errData[i];
        ...
        return {
            ...
            fmtTitle: function(){
                if(typeof _this.formatter.title === 'function'){
                    const ftitle = _this.formatter.title(data);
                    let tt = data[_this.response.title];
            ...
            checkArr: function(){
                let checkArr = [];
                let checkArrData = data[_this.response.checkArr];
            ...
            recordData: function(){
                const recordData = _this.record ? event.cloneObj(data, [_this.response.treeId,
    ...
    DTree.prototype.getDom = function(treeId, parentId, title, fmtTitle, last, ficonClass, iconClass, checkArr, level, spread, disabled, hide) {
        ...
        return {
            ...
            node: function() {	// 二级图标样式
                // 获取图标的变量
                const nodeIcon = _this.nodeIcon,
                    leafIcon = _this.leafIcon;

                let sleafIconLeaf = _this.usefontStyle.snode.leaf,
            ...
            checkbox: function() {	// 复选框
                ...
                if(flag){
                    let result = "<div class='" + LI_DIV_CHECKBAR + "' data-id='" + treeId + "' dtree-id='" + rootId + "'>";
                    if(checkArr && checkArr.length > 0){

                        for (let i = 0; i < checkArr.length; i++) {
                            const checkData = checkArr[i];
                            const checked = checkData[_this.response.checked];
                            const type = checkData[_this.response.type];
                            let CHOOSE_CLASS = _this.usefontStyle.checkbox.out;
                            if (checked === "2") {	//半选择
    ...
    DTree.prototype.replaceDom = function($div, treeId, last, spread, disabled, hide) {
        ...
        return {
            ...
            text: function(title) {	// 文字显示
                let disClass = "";
                if(disabled){disClass = NAV_DIS;}
                const cite = "<cite class='" + LI_DIV_TEXT_CLASS + " " + disClass + "' data-id='" + treeId + "' data-leaf='" + (last ? "leaf" : "node") + "' dtree-disabled='" + disabled + "' >" + title + "</cite>";
                _this.getNodeDom($div).cite().replaceWith($(cite));
            },
            ul: function() {	//子节点ul
                const ul = last ? "<ul class='" + LI_NAV_CHILD + "' data-id='" + treeId + "' dtree-id='" + rootId + "'></ul>" :
                    (spread ? "<ul class='" + LI_NAV_CHILD + " " + NAV_SHOW + "' data-id='" + treeId + "' dtree-id='" + rootId + "'></ul>" : "<ul class='" + LI_NAV_CHILD + "' data-id='" + treeId + "' dtree-id='" + rootId + "'></ul>");
    ...
    DTree.prototype.getLiItemDom =  function(treeId, parentId, title, fmtTitle, last, ficonClass, iconClass, checkArr, level, spread, disabled, hide, basicData, recordData, isRoot) {
        ...
        const dom = _this.getDom(treeId, parentId, title, fmtTitle, last, ficonClass, iconClass, checkArr, level, spread, disabled, hide);
        basicData = (basicData === "{}") ? "" : basicData;
        recordData = (recordData === "{}") ? "" : recordData;
        let div = "<div class='" + LI_DIV_ITEM + " " + _this.style.item + "' data-id='" + treeId + "' dtree-id='" + rootId + "' dtree-click='" + eventName.itemNodeClick + "' data-basic='" + basicData + "' data-record='" + recordData + "' dtree-disabled='" + disabled + "' dtree-hide='" + hide + "' ";
        if(_this.toolbar){
        ...
        const li = ["<li " + "class='" + LI_CLICK_CHECKBAR + " " + LI_NAV_ITEM + " " + hideClass + " " + lineClass + "'" + "data-id='" + treeId + "'" + "data-pid='" + (isRoot == "root" ? ((typeof parentId !== undefined && parentId != "") ? parentId : "-1") : parentId) + "'" + "dtree-id='" + rootId + "'" + "data-index='" + level + "'" + "dtree-hide='" + hide + "'" + ">" +
        div,
            dom.fnode(),
            dom.node(),
            dom.checkbox(),
    ...
    DTree.prototype.dataInit = function(chooseId){
        const _this = this;
        const $div = _this.obj.find("div[data-id='" + chooseId + "']");
        if($div && $div.length > 0) {
        	...
        	_this.setNodeParam($div);
        	// 将该节点的父节点全部展开
            const $li_parents = $div.parents("." + LI_NAV_ITEM);
    ...
    DTree.prototype.navThis = function(id){
        const _this = this;
        const $div = (typeof id === 'object') ? id : (_this.obj.find("div[dtree-click='" + eventName.itemNodeClick + "'][data-id='" + id + "']").length === 0) ? null : _this.obj.find("div[dtree-click='" + eventName.itemNodeClick + "'][data-id='" + id + "']");
        if($div != null) {
            _this.cancelNavThis();
            $div.addClass(NAV_THIS);
            $div.addClass(_this.style.itemThis);
    ...
    DTree.prototype.accordionUL = function($ul) {
        ...
        if(_this.accordion) {
            ...
            const $divs = $ul.closest("li[data-index]").siblings("li[data-index]").children("ul[data-id]").prev("div");
            if($divs.length && $divs.length > 0) {
                for (let i=0; i<$divs.length; i++) {
                    const $div = $($divs[i]);
                    const $i_fnode = _this.getNodeDom($div).fnode(),
                        $i_snode = _this.getNodeDom($div).snode();
    ...
    DTree.prototype.setDisabledNodes = function(disabledIds){
        ...
        if(disabledIds.length) {
        	for (let i=0; i<disabledId.length; i++) {
    ...
    DTree.prototype.setDisabledNode = function($div){
        const _this = this;
        const $i = $div.children("div." + LI_DIV_CHECKBAR).children("i[data-par]");
        const $cite = $div.children("cite[data-leaf]");
        if($div.attr("dtree-disabled") !== "true") {
    ...
    DTree.prototype.setHideNode = function($div){
        const _this = this;
        const $li = $div.parent("li[dtree-hide]");
        if($div.attr("dtree-hide") !== "true") {
    ...
    DTree.prototype.partialRefreshAdd = function($div, data){
        ...
        const $icon_i = $div.find("i[data-spread]");
        ...
        if(data) {
            if(data.length && data.length > 0) {
                ...
                if(parseData.treeId()){
                    const level = parseInt($div.parent("li").attr("data-index")) + 1;
    ...
    DTree.prototype.partialRefreshEdit = function($div, data){
        ...
        if(data) {
            if(typeof data === 'object') {
                ...
                if(parseData.treeId()){
                    const replaceDom = _this.replaceDom($div, parseData.treeId(), parseData.last(0), parseData.spread(), parseData.disabled(), parseData.hide());
                    replaceDom.div();
    ...
    DTree.prototype.partialRefreshDel = function($div){
        ...
        if($p_ul.children("li").length === 0){
            const $icon_i = $p_div.find("i[data-spread]");
    ...
    DTree.prototype.fuzzySearch = function (value) {
        ...
        let cites = [];
        const filterMode = _this.filterMode;
        /*if (filterMode == 'default') {
            cites = _this.obj.find("li>div cite:contains('" + value + "')");
        }*/
        cites = _this.obj.find("li>div cite:contains('" + value + "')");
        cites.each(function (index, cite) {
            const li = $(cite).parent().parent();
            const curNode = _this.getParam(li.attr("data-id"));
    ...
    DTree.prototype.chooseDataInit = function(chooseIds){
        ...
        if(chooseId.length) {
        	let i;
// 说明是数组
        	for (i = 0; i<chooseId.length; i++) {
                const choose = chooseId[i];
        	...
        	// 遍历二维数组解析数据初始化选中
        	for(i = 0; i<chooseIdArrs.length; i++) {
                const chooseIdArr = chooseIdArrs[i];
                for (let j=0; j<chooseIdArr.length; j++) {
            		$i = _this.obj.find("i[dtree-click='"+eventName.checkNodeClick+"'][data-type="+i+"][data-id="+chooseIdArr[j]+"]");
            		_this.checkStatus($i).check();
                }
        	}
        	
        } else {
            $i = _this.obj.find("i[dtree-click='" + eventName.checkNodeClick + "'][data-id=" + chooseId + "]");
            _this.checkStatus($i).check();
        }
        
        
        // 展开选中节点的父节点
        const $li_parents = _this.obj.find("i[dtree-click='" + eventName.checkNodeClick + "'][data-checked='1']").parents("." + LI_NAV_ITEM);
        $li_parents.children("ul").addClass(NAV_SHOW);
        $li_parents.children("."+LI_DIV_ITEM).children("i[data-spread]."+event.trimToDot(_this.usefontStyle.fnode.node.close)).addClass(_this.usefontStyle.fnode.node.open);
        $li_parents.children("."+LI_DIV_ITEM).children("i[data-spread]."+event.trimToDot(_this.usefontStyle.fnode.node.close)).removeClass(_this.usefontStyle.fnode.node.close);
        $li_parents.children("."+LI_DIV_ITEM).children("i[data-spread]."+event.trimToDot(_this.usefontStyle.snode.node.close)).addClass(_this.usefontStyle.snode.node.open);
    ...
    DTree.prototype.checkAllOrNot =  function($i) {
        ...
        if ($i.attr("data-checked") === "1") {
            ...
            // 处理子级节点的选中状态
            $child_li_i = $child_li.find(">."+LI_DIV_ITEM+">."+LI_DIV_CHECKBAR+">i[data-type='"+dataType+"']");
            _this.checkStatus($child_li_i).noCheck();

            // 处理父级节点的选中状态
            for (; i < item.length; i++) {
                const flag = item.eq(i).find(">." + LI_NAV_CHILD + " ." + LI_DIV_CHECKBAR + ">i[data-type='" + dataType + "'][data-checked='1']").length;
                if (flag === 0) {
                    //把父级去掉选中
                    $item_i = item.eq(i).find(">."+LI_DIV_ITEM+">."+LI_DIV_CHECKBAR+">i[data-type='"+dataType+"']");
    ...
    DTree.prototype.checkAllOrNoallOrNot =  function($i) {
        ...
        const _this = this;
        //$i 当前点击的checkbox
        const $div = $i.closest("." + LI_DIV_ITEM),
        ...
        if ($i.attr("data-checked") === "1") {	//当前复选框为选中状态，点击后变为未选中状态
            ...
            item = $parent_li;
            for (; i < item.length; i++) {
                const flag1 = item.eq(i).find(">." + LI_NAV_CHILD + " ." + LI_DIV_CHECKBAR + ">i[data-type='" + dataType + "'][data-checked='1']").length;
                const flag2 = item.eq(i).find(">." + LI_NAV_CHILD + " ." + LI_DIV_CHECKBAR + ">i[data-type='" + dataType + "']").length;
                $item_i = item.eq(i).find(">." + LI_DIV_ITEM + ">." + LI_DIV_CHECKBAR + ">i[data-type='" + dataType + "']");
                if (flag1 !== flag2) {
                    // 父级复选框半选
                    _this.checkStatus($item_i).noallCheck();
    ...
    DTree.prototype.checkOnly = function($i) {
        ...
        const checked = $i.attr("data-checked");
        // 将全部节点全部设为未选中状态
        const $all_i = _this.obj.find("i[data-checked]");
    ...
    DTree.prototype.changeCheck = function($i) {
        ...
        // 获取复选框选中节点的内容
        const checkbarNodes = _this.setAndGetCheckbarNodesParam(true);
    ...
    DTree.prototype.initNoAllCheck = function(){
        const _this = this;
        //1.获取所有选中节点
        const $is = _this.obj.find("i[data-checked='1']");
        if($is.length > 0){
            for (let key = 0; key < $is.length; key++) {
                const $i = $($is[key]),
                    dataPar = $i.attr("data-par"),
                    dataType = $i.attr("data-type"),
                    $li = $i.closest(dataPar),		//当前checkbox的上级li节点
                ...
                // 处理父级节点的选中状态
                let i = 1, item = $parent_li;
                for (; i < item.length; i++) {
                    const flag1 = item.eq(i).find(">." + LI_NAV_CHILD + " ." + LI_DIV_CHECKBAR + ">i[data-type='" + dataType + "'][data-checked='1']").length;
                    const flag2 = item.eq(i).find(">." + LI_NAV_CHILD + " ." + LI_DIV_CHECKBAR + ">i[data-type='" + dataType + "']").length;
                    const $item_i = item.eq(i).find(">." + LI_DIV_ITEM + ">." + LI_DIV_CHECKBAR + ">i[data-type='" + dataType + "']");
                    if (flag1 !== flag2) {
                        // 父级复选框半选
                        _this.checkStatus($item_i).noallCheck();
                    } else {
    ...
    DTree.prototype.setAndGetCheckbarNodesParam = function(requestParamFlag) {
        ...
        if (_this.checkbarData === "change"){	//记录变更数据
            _this.obj.find("i[data-par][dtree-disabled='false']").each(function(){
                const $i = $(this), $div = $i.closest("." + LI_DIV_ITEM);
    ...
    DTree.prototype.getCheckbarNodeParam = function($div, $i){
        ...
        temp_node.leaf = _this.getNodeDom($div).cite().attr("data-leaf") === "leaf";
        temp_node.level = _this.getNodeDom($div).parentLi().attr("data-index");
        temp_node.spread = _this.getNodeDom($div).fnode().attr("data-spread") === "open";

        let basicData = $div.attr("data-basic");
        if(basicData) {
    ...
    DTree.prototype.getCheckbarJsonArrParam = function(){
        ...
        // 获取全部复选框选中节点
        const params = _this.setAndGetCheckbarNodesParam(false);
    ...
    DTree.prototype.clickNodeCheckbar = function(nodeId){
        const _this = this;
        const $checkbar = _this.getNodeDom(nodeId).checkbox();
        _this.temp = [$checkbar];
		_this.changeCheck();
    }
    ...
    DTree.prototype.checkAllNode = function(){
        const _this = this;
        const $i = _this.obj.find("i[data-par][data-checked!='1']");
    ...
    DTree.prototype.cancelCheckedNode = function(){
        const _this = this;
        const $i = _this.obj.find("i[data-par][data-checked!='0']");
    ...
    DTree.prototype.removeCheckedNode = function(){
        const _this = this;
        const len = _this.obj.find("i[data-par][data-checked='1']").length;
        if(len === 0){
            ...
            layer.confirm('确定要删除选中节点？', {icon: 3, title:'删除选中节点'}, function(index1){
                const flag = _this.menubarFun.remove(_this.checkbarNode);
                if(flag){
    ...
    DTree.prototype.menubarMethod = function(){
        ...
        return {
            openAllNode: function(obj){  // 展开所有节点
                const $ulNode = obj || _this.obj.children("li").children("ul");
                // 遍历所有ul子节点
            ...
            searchNode: function(){//模糊查询该值，展开该值节点
                layer.prompt({
                    ...
                    if (value) {
                        const flag = _this.searchNode(value);
                        if (!flag) {
            ...
            extMethod: function(menuId, $div, flag){
                ...
                if(_this.menubar && _this.menubarTips.group && _this.menubarTips.group.length > 0 && flag === "group"){
                    for(i = 0; i<_this.menubarTips.group.length; i++){
                        ext = _this.menubarTips.group[i];
                ...
                if(_this.menubar && _this.menubarTips.toolbar && _this.menubarTips.toolbar.length > 0 && flag === "toolbar"){
                    for(i = 0; i<_this.menubarTips.toolbar.length; i++){
                        ext = _this.menubarTips.toolbar[i];
                        if (menuId === ext.menubarId){
                            ext.handler(_this.getRequestParam(_this.getNodeParam($div), $div));
                            break;
                        }
                ...
                if(_this.menubar && _this.menubarTips.freedom && _this.menubarTips.freedom.length > 0 && flag === "freedom"){
                    for(i = 0; i<_this.menubarTips.freedom.length; i++){
                        ext = _this.menubarTips.freedom[i];
                        if (menuId === ext.menubarId){
                            ext.handler(_this.getRequestParam(_this.getNodeParam($div), $div));
                            break;
                        }
    ...
    DTree.prototype.menubarListener = function(menuId, flag){
        const _this = this;
        const $div = _this.getNodeDom().nowDiv();
        switch (menuId) {
    ...
    DTree.prototype.searchNode = function(value){
        ...
        _this.obj.find("cite[data-leaf]").each(function(){
            ...
            if(html.indexOf(value) > -1){
                ...
                let i = 0;
                $nthis.parents("li").each(function(){
                    const html2 = $(this).find("cite[data-leaf]").html();
    ...
    DTree.prototype.getToolbarDom = function(){
        let i;
        const _this = this;
        const toolbarShow = _this.toolbarShow,
            toolbarExt = _this.toolbarExt,
            toolbarWay = _this.toolbarWay;

        if(toolbarShow.length > 0){
            for(i = 0; i<toolbarShow.length; i++){
                const show = toolbarShow[i];
                if(show === "pulldown"){
        ...
        if(toolbarExt.length > 0){
            for(i = 0; i<toolbarExt.length; i++){
                const ext = toolbarExt[i];
    ...
    DTree.prototype.setToolbarDom = function(){
        const _this = this;
        const toolbarWay = _this.toolbarWay;
    ...
    DTree.prototype.dynamicToolbarDom = function($cite){
        ...
        if($cite.next("em."+TOOLBAR_TOOL_EM).length === 0) {
            const $div = $cite.parent("div");
            const param = _this.getRequestParam(_this.getTempNodeParam($div));
            const toolbarMenus = _this.toolbarFun.loadToolbarBefore(event.cloneObj(_this.toolbarMenu), param, $div);
            const hideCls = (toolbarWay === "follow") ? NAV_HIDE : "";
            const em = ["<em class='" + TOOLBAR_TOOL_EM + " " + hideCls + "'>"];
            if(toolbarMenus){
    ...
    DTree.prototype.toolbarHide = function() {
        const _this = this;
        if(_this.toolbar && _this.toolbarWay === "contextmenu") {
            const $toolBarDiv = _this.obj.prevAll('div#dtree_toolbar_' + _this.obj[0].id);
        ...
        return {
            pulldown: function(obj){ // 展开当前点击节点的下面全部节点
                ...
                for (let i = 0; i < $ulNode.length; i++) {
                    ...
                    const $childUl = $ul.children("li").children("ul");
    ...
    DTree.prototype.toolbarListener = function(tool, $div) {
        ...
        switch (tool) {
            ...
            case defaultTool.addTool:
                let content = _this.loadToolBar(title, defaultTool.addTool);
                ...
                layer.open({
                    ...
                    success: function(layero, index){
                        ...
                        form.on("submit(dtree_addNode_" + _this.obj[0].id + "_form)",function(data){
                            var data = data.field;
                            let parentId = $div.attr("data-id"),
                                id = $div.attr("data-id") + "_node_" + $ul[0].childNodes.length,
                                leaf = true,
                                checked = "0",
                                level = parseInt($p_li.attr("data-index")) + 1;

                            const type = _this.response.type;
                            checked = _this.response.checked;
                            // 创建子节点的DOM，添加子节点
                            const checkArr = [];
                            if (_this.checkArrLen > 0) {
                                for (let i = 0; i < _this.checkArrLen; i++) {
                            ...
                            // 重新赋值
                            const $addDiv = $ul.find("div[data-id='" + id + "']");
                            node = _this.getNodeParam($addDiv);

                            //获取组装后的requestNode,组合参数
                            let requestNode = _this.getRequestParam(node);
            ...
            case defaultTool.editTool:
                content = _this.loadToolBar(title, defaultTool.editTool);
            ...
            case defaultTool.delTool:
                layer.confirm('确定要删除该'+_this.toolbarStyle.title+'？', {icon: 3, title:'删除'+_this.toolbarStyle.title}, function(index){
                    const node = _this.getNodeParam($div);
            ...
            default:
                if(_this.toolbarExt.length > 0){
                    for(let i=0; i<_this.toolbarExt.length; i++){
                        const ext = _this.toolbarExt[i];
    ...
    DTree.prototype.loadToolBar = function(title, name){
        ...
        const _this = this;
        const toolbarShow = _this.toolbarShow;
        const nodeBarContents = _this.toolbarBtn;
        ...
        let j;switch (name) {
            case defaultTool.addTool:
                ...
                //2. 用户自定义的节点内容
                const addNodeBar = ['<div class="' + TOOLBAR_TOOL + '"><form class="layui-form layui-form-pane" lay-filter="dtree_addNode_' + _this.obj[0].id + '_form">'];
                ...
                if(nodeBarContents != null && nodeBarContents.length > 0){
                    if(nodeBarContents[0] != null && nodeBarContents[0] !== undefined && nodeBarContents[0].length > 0){
                        const addNodeBarContents = nodeBarContents[0];
                        // 1. 检查是否包含了now、newly、btn这三个默认项,将其他元素依次排列，将特殊元素至于栈顶
                        for(i = 0; i<addNodeBarContents.length; i++){
                            defElem = addNodeBarContents[i].defElem;
                ...
                // 2. 遍历生成全部表单标签
                for(j = 0; j<addNodeBarDef.length; j++){
                    type = addNodeBarDef[j].type;
                    if(!type){type = "text";}
                    switch (type) {
                        case "text":
                ...
                const addBtn = ['<div class="layui-form-item">', '<div class="layui-input-block" style="margin-left:0px;text-align:center;">'];
                // 3.遍历生成按钮
                for(j = 0; j<addNodeBarDef.length; j++){
                    type = addNodeBarDef[j].type;
                    if(!type){type = "text";}
            ...
            case defaultTool.editTool:
                ...
                const editNodeBar = ['<div class="' + TOOLBAR_TOOL + '"><form class="layui-form layui-form-pane" lay-filter="dtree_editNode_' + _this.obj[0].id + '_form">'];
                //2. 用户自定义的节点内容
                if(nodeBarContents != null && nodeBarContents.length > 0){

                    if(nodeBarContents[1] != null && nodeBarContents[1] !== undefined && nodeBarContents[1].length > 0){
                        const editNodeBarContents = nodeBarContents[1];
                        // 1. 检查是否包含了now、newly、btn这三个默认项,将其他元素依次排列，将特殊元素至于栈顶
                        for(i = 0; i<editNodeBarContents.length; i++){
                            defElem = editNodeBarContents[i].defElem;
                            if(defElem === "nowChoose") {
                ...
                // 2. 遍历生成全部表单标签
                for(j = 0; j<editNodeBarDef.length; j++){
                    type = editNodeBarDef[j].type;
                    if(!type){type = "text";}
                    switch (type) {
                        case "text":
                ...
                const editBtn = ['<div class="layui-form-item">', '<div class="layui-input-block" style="margin-left:0px;text-align:center;">'];
                // 3.遍历生成按钮
                for(j = 0; j<editNodeBarDef.length; j++){
                    type = editNodeBarDef[j].type;
                    if(!type){type = "text";}
    ...
    DTree.prototype.loadToolBarDetail = function(nodeBarContents){
        const _this = this;
        const readonly = (typeof (nodeBarContents.readonly) === "boolean") ? nodeBarContents.readonly : false;
        const disabled = (typeof (nodeBarContents.disabled) === "boolean") ? nodeBarContents.disabled : false;
        const id = nodeBarContents.id ? nodeBarContents.id : "";
        const name = nodeBarContents.name ? nodeBarContents.name : "";
        const val = nodeBarContents.value ? nodeBarContents.value : "";
        const verify = nodeBarContents.verify ? nodeBarContents.verify : "";
        const placeholder = nodeBarContents.placeholder ? nodeBarContents.placeholder : val;
        return{
            text: function(){
            ...
            select: function(){
                const optionsData = (typeof nodeBarContents.optionsData === 'object') ? nodeBarContents.optionsData : nodeBarContents.optionsData();
                let options = "";
            ...
            submit: function(){
                const filter = nodeBarContents.filter;
    ...
    DTree.prototype.changeTreeNodeAdd = function(returnID){
        const _this = this;
        const temp = _this.temp;
        const id = temp[0], $ul = temp[1], $div = temp[2], level = temp[3];
        let flag = false;
        if(returnID){
            let $thisDiv = _this.obj.find("[data-id='" + id + "']");
            if(typeof returnID === "object"){
                // 如果是JSON格式数据，则将当前DIV删除，重新建造DIV
                $thisDiv.remove();
                const parseData = _this.parseData(returnID);
    ...
    DTree.prototype.changeTreeNodeEdit = function(returnID){
        const _this = this;
        const temp = _this.temp;
        const $cite = temp[0], $div = temp[1], title = temp[2], $p_div = temp[3];
        const flag = false;
        if(returnID){
            if(typeof returnID === "object"){
                const parseData = _this.parseData(data);
    ...
    DTree.prototype.loadIframe = function($div, iframeParam) {
        const _this = this;
        const $cite = _this.getNodeDom($div).cite();
        if (!_this.useIframe) {		// 启用iframe
            return false;
        }
        const iframeElem = _this.iframeElem,
            iframeUrl = _this.iframeUrl,
            iframeLoad = _this.iframeLoad;

        const flag = iframeLoad === "leaf" ? $cite.attr("data-leaf") === "leaf" : true;
        ...
        if (flag) {
            if ($(iframeElem).length > 0) {		//iframe存在
                ...
                let param = AjaxHelper.serialize("?", iframeParam);
                if(iframeUrl.indexOf("?")> -1){
                    param = "&"+param.substring(1, param.length);
                }
                const url = iframeUrl + param;
    ...
    DTree.prototype.getIframeRequestParam = function(nodes){
        let key;
        const _this = this;
        const request = _this.iframeRequest,
        ...
        for (key in defaultRequestNames) {
            const paramName = defaultRequestNames[key];
            const paramValue = node[key];
    ...
    DTree.prototype.setNodeParam = function($div) {
        ...
        let recordData = $div.attr("data-record");
        if(recordData) {
        	recordData = JSON.parse(event.unescape(recordData));
        }
        ...
        if (_this.getNodeDom($div).checkbox()) {
            let dataTypes = "", checkeds = "", initcheckeds = "";
    ...
    DTree.prototype.getFilterRequestParam = function(requestParam){
        const _this = this;
        const filterRequest = _this.filterRequest;
    ...
    DTree.prototype.getAllParentParam = function(id){
        const _this = this;
        // 获取当前div，如果id就是一个dom，则就是这个，如果不是则进行选择。如果选不中则为null
        let $div = (typeof id === 'object') ? id : (_this.obj.find("div[dtree-click='" + eventName.itemNodeClick + "'][data-id='" + id + "']").length === 0) ? null : _this.obj.find("div[dtree-click='" + eventName.itemNodeClick + "'][data-id='" + id + "']");
        const arr = [];
        if($div != null){
            const level = _this.getTempNodeParam($div).level;
            for(let i=1; i<level; i++){  // 从1开始遍历，如果level等于1说明是根节点
    ...
    DTree.prototype.callbackData = function(){
        ...
        return {
            ...
            childNode: function($div){	// 获取下级节点值
                const $childDivs = $div.next("ul").find("li." + LI_NAV_ITEM + " div." + LI_DIV_ITEM);
                const childNode = [];
            ...
            parentNode: function($div){	// 获取上级节点值
                const pId = _this.getNodeDom($div).parentLi().attr("data-pid");
                const $pdiv = _this.obj.find("div[data-id='" + pId + "']");
    ...
    DTree.prototype.bindBrowserEvent = function(){
        ...
        _this.obj.on("click", "i[data-spread]", function(event) {
            ...
            layui.event.call(this, MOD_NAME, "changeTree("+$(_this.obj)[0].id+")",  {
                ...
                show: _this.callbackData().dom($i).attr("data-spread") === "open"
            });
        });
        ...
        _this.obj.on("click", "div[dtree-click='"+eventName.itemNodeClick+"'][dtree-disabled='false']", function(event) {
            ...
            if (_this.useIframe) {
                const iframeParam = _this.getFilterRequestParam(_this.getIframeRequestParam(node));
                const flag = _this.loadIframe($div, iframeParam);	// 加载iframe
                if (flag) {
        ...
        if(_this.checkbar) {
            ...
            _this.obj.on("click", "i[dtree-click='"+eventName.checkNodeClick+"'][dtree-disabled='false']", function(event) {
                ...
                const $i = $(this),
                    ...
                    node = _this.getNodeParam($div);
                // 复选框选中前的回调
                const flag = _this.checkbarFun.chooseBefore($i, _this.getRequestParam(node));
                _this.temp = [$i];
        ...
        if(_this.toolbar) {
            if(_this.toolbarWay === "contextmenu") {
                ...
                _this.obj.prevAll('div#dtree_toolbar_'+_this.obj[0].id).on("click", "a[dtree-tool]", function(event) {
                    event.stopPropagation();
                    const $div = _this.getNodeDom().nowOrRootDiv(),
                        node = _this.getNodeParam($div);
                    _this.toolbarHide();
                    const tool = $(this).attr("dtree-tool");
        ...
        if(_this.select) {
            ...
            $("div[dtree-id='" + rootId + "'][dtree-select='"+_this.selectDiv+"']").on("click", function(event){
                ...
                const $card = $("div[dtree-id='" + rootId + "'][dtree-card='" + _this.selectCardDiv + "']");

                $card.toggleClass("dtree-select-show layui-anim layui-anim-upbit");
                const top = $(this).offset().top + $(this).outerHeight() - $WIN.scrollTop() - 5,
...
//                console.log("winHeight = " + winHeight);
    ...
    const dtree = {
        ...
        render: function (options) {	// 初始化树
            let dTree = null;
            const id = event.getElemId(options);
            if (id === "") {
