layui.define(['table', 'laypage','jquery', 'element'], function(exports) {
	...
	function createComponent(elem,linenum,data) {
		...
		let content = createCards(elem, linenum, data);
		const page = "";
	...
	function createCards(elem, linenum,data) {
		let content = "<div class='layui-row layui-col-space30'>";
	...
	function initData(tempData, option) {
		...
		const dataList = tempData[option.response.dataName];
		if(!dataList){
            return data;
        }
		data.data = [];
	...
	const tt = {
		...
		getChecked: function (id) {
			const option = _instances[id].option;
			const data = option.checkedItem;
			const item = {};
			if (!data) {
				return null;
			}
